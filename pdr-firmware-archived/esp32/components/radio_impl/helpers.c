...
void blockingTransmitBuffer(uart_port_t uartPort, const uint8_t* const buffer,
                            uint16_t bufferLength) {
    uart_write_bytes(uartPort, buffer, bufferLength);
    if (uart_wait_tx_done(uartPort, d_uartTxTimeout_ms / portTICK_PERIOD_MS) ==
        ESP_ERR_TIMEOUT) {
        verbPrintf("Failed to send!\r\n");
    } else
        verbPrintf("Sent successfully! \r\n");
}
...
bool waitForAck(uart_port_t uartPort, uint16_t timeout_ms,
    ...
    int len = uart_read_bytes(uartPort, &buf, sizeof(buf),
