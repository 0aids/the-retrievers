    ...
    while (*format)
    {
        ...
        switch (type)
        {
            ...
            { // NMEA talker identifier and type (union minmea_type
                ...
                union minmea_type* buf =
                    va_arg(ap, union minmea_type*);
                memcpy(buf, field + 1,
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "tTfffifff", &frame->type,
                     &frame->time, &frame->err_latitude,
                     &frame->err_longitude, &frame->err_altitude,
                     &frame->svid, &frame->prob, &frame->bias,
                     &frame->stddev))
        return false;
    if (memcmp(frame->type.sentence_id, "GBS",
               sizeof(frame->type.sentence_id)))
        return false;
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "tTcfdfdffDfd", &frame->type,
                     &frame->time, &validity, &frame->latitude,
                     &latitude_direction, &frame->longitude,
                     &longitude_direction, &frame->speed,
                     &frame->course, &frame->date, &frame->variation,
                     &variation_direction))
        return false;
    if (memcmp(frame->type.sentence_id, "RMC",
               sizeof(frame->type.sentence_id)))
        return false;
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(
            sentence, "tciiiiiiiiiiiiifff", &frame->type,
            &frame->mode, &frame->fix_type, &frame->sats[0],
            &frame->sats[1], &frame->sats[2], &frame->sats[3],
            &frame->sats[4], &frame->sats[5], &frame->sats[6],
            &frame->sats[7], &frame->sats[8], &frame->sats[9],
            &frame->sats[10], &frame->sats[11], &frame->pdop,
            &frame->hdop, &frame->vdop))
        return false;
    if (memcmp(frame->type.sentence_id, "GSA",
               sizeof(frame->type.sentence_id)))
        return false;
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "tfdfdTc;c", &frame->type,
                     &frame->latitude, &latitude_direction,
                     &frame->longitude, &longitude_direction,
                     &frame->time, &frame->status, &frame->mode))
        return false;
    if (memcmp(frame->type.sentence_id, "GLL",
               sizeof(frame->type.sentence_id)))
        return false;
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "tTfffffff", &frame->type,
                     &frame->time, &frame->rms_deviation,
        ...
        return false;
    if (memcmp(frame->type.sentence_id, "GST",
               sizeof(frame->type.sentence_id)))
        return false;
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "tiii;iiiiiiiiiiiiiiii", &frame->type,
                     &frame->total_msgs, &frame->msg_nr,
                     &frame->total_sats, &frame->sats[0].nr,
    ...
    if (memcmp(frame->type.sentence_id, "GSV",
               sizeof(frame->type.sentence_id)))
        return false;
    return true;
}
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "t;fcfcfcfcc", &frame->type,
                     ...
                     &c_kph, &c_faa_mode))
        return false;
    if (memcmp(frame->type.sentence_id, "VTG",
               sizeof(frame->type.sentence_id)))
        return false;
                      ...
                      const char*                 sentence)
{
    ...
    if (!minmea_scan(sentence, "tTiiiii", &frame->type, &frame->time,
                     ...
                     &frame->minute_offset))
        return false;
    if (memcmp(frame->type.sentence_id, "ZDA",
               sizeof(frame->type.sentence_id)))
        return false;
                   ...
                   const struct minmea_time* time_)
{
    ...
    if (minmea_getdatetime(&tm, date, time_))
        return -1;
    // time_t timestamp = timegm(&tm); /* See README.md if your system lacks
    // timegm(). */
