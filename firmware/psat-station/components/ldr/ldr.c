...
void ldr_setup(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(
        ldr_adcHandlers_g.adc1, ldr_adc1Chan0_d, &config));
    ...
    adcCalibrationInit(ADC_UNIT_1, ldr_adc1Chan0_d, ldr_adcAtten_d,
                       &ldr_adcHandlers_g.adc1CaliChan0);
}
...
int ldr_getVoltage(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_read(ldr_adcHandlers_g.adc1,
                                     ldr_adc1Chan0_d, &adcRaw));
    ESP_ERROR_CHECK(adc_cali_raw_to_voltage(
        ldr_adcHandlers_g.adc1CaliChan0, adcRaw, &voltage));
...
char ldr_stateConfigurationBuffer_g
    [ldr_stateConfigurationBufferSize_d] = {0};
...
ldr_state_t ldr_queryState(void)
{
    ...
    size_t trueSize =
        (ldr_stateConfigurationBufferSize_d - 1 < size) ?
        ldr_stateConfigurationBufferSize_d - 1 :
        size;
