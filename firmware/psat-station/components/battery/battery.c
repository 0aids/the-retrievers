...
static bool adcCalibrationInit(adc_unit_t unit, adc_channel_t channel,
                               adc_atten_t        atten,
                               adc_cali_handle_t* outHandle)
{
    ...
    if (!calibrated)
    {
        ESP_LOGI(battery_tag_c, "calibration scheme version is %s",
                 "Curve Fitting");
...
void battery_setup(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_new_unit(
        &initConfigBattery, &battery_adcHandlers_g.adcHandle));
    ...
    ESP_ERROR_CHECK(
        adc_oneshot_config_channel(battery_adcHandlers_g.adcHandle,
                                   battery_adc1Chan0_d, &config));
    ...
    adcCalibrationInit(initConfigBattery.unit_id, battery_adc1Chan0_d,
                       config.atten,
                       &battery_adcHandlers_g.adcCaliChan0);
}
...
int battery_getVoltage(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_read(battery_adcHandlers_g.adcHandle,
                                     battery_adc1Chan0_d, &adcRaw));
    ESP_ERROR_CHECK(adc_cali_raw_to_voltage(
        battery_adcHandlers_g.adcCaliChan0, adcRaw, &voltage));

    return voltage;
}

char battery_stateConfigBuffer_g[battery_stateConfigBufferSize_d] = {
    0};
...
battery_state_t battery_queryState(void)
{
    ...
    size_t trueSize = (battery_stateConfigBufferSize_d - 1 < size) ?
        battery_stateConfigBufferSize_d - 1 :
        size;
...
{
    ESP_ERROR_CHECK(
        adc_oneshot_del_unit(battery_adcHandlers_g.adcHandle));
    ESP_LOGD(battery_tag_c, "deregister %s calibration scheme",
             "Line Fitting");
