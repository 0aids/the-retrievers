    ...
    [button_id_landing] =
        {
            ...
            .event = psatFSM_eventType_landingConfirmed,
        },
    [button_id_prelaunch] =
        {
            .intr_type = GPIO_INTR_NEGEDGE,
...
static void IRAM_ATTR button_isr(void* arg) {
    ...
    psatFSM_event_t event = {.type = buttonConfig_c[id].event, .global = false};
    psatFSM_postEventISR(&event);
}

void button_init(void) {
    const config_pinConfig_t* pins = config_getPinConfig();
...
void button_enable(button_id_e id) {
    gpio_isr_handler_add(buttonConfig_c[id].pin, button_isr, (void*)id);
