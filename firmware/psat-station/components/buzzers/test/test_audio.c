...
{
    buzzer_init();
    gpio_set_direction(CFG_BUZZER_PIN_d, GPIO_MODE_INPUT_OUTPUT);

    TEST_ASSERT_EQUAL(LOW, gpio_get_level(CFG_BUZZER_PIN_d));
}
...
TEST_CASE("Buzzer GPIO pin goes LOW HIGH LOW on beep", "[audio]")
{
    ...
    gpio_set_direction(
        CFG_BUZZER_PIN_d,
        GPIO_MODE_INPUT_OUTPUT); // overwrite default which is OUTPUT only

    int before, during, after;
    before = gpio_get_level(CFG_BUZZER_PIN_d);

    buzzer_beep(duration_ms);
    during = gpio_get_level(CFG_BUZZER_PIN_d);

    vTaskDelay(pdMS_TO_TICKS(
        duration_ms +
        REACTION_TIME)); // give it about 100ms extra to go LOW
    after = gpio_get_level(CFG_BUZZER_PIN_d);
...
TEST_CASE(
    "Buzzer beep mid another beep uses most recent beep (extending)",
    "[audio]")
{
    buzzer_init();
    gpio_set_direction(CFG_BUZZER_PIN_d, GPIO_MODE_INPUT_OUTPUT);
    ...
    buzzer_beep(2500);
    vTaskDelay(pdMS_TO_TICKS(REACTION_TIME));
    TEST_ASSERT_EQUAL(HIGH, gpio_get_level(CFG_BUZZER_PIN_d));
...
TEST_CASE(
    "Buzzer beep mid another beep uses most recent beep (reducing)",
    "[audio]")
{
    buzzer_init();
    gpio_set_direction(CFG_BUZZER_PIN_d, GPIO_MODE_INPUT_OUTPUT);
    ...
    buzzer_beep(
        2500); // original beep is 500ms but we will this time lower it
    vTaskDelay(pdMS_TO_TICKS(250));
