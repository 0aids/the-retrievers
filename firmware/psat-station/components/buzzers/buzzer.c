...
static void beepTimerCb(void* arg) {
    gpio_set_level(config_getPinConfig()->buzzerPin, 0);
    buzzerActive_s = false;
}

void buzzer_init(void) {
    const int buzzerPin = config_getPinConfig()->buzzerPin;
    gpio_config_t io_conf = {.pin_bit_mask = 1ULL << buzzerPin,
...
void buzzer_turnOn(void) {
    gpio_set_level(config_getPinConfig()->buzzerPin, 1);
    buzzerActive_s = true;
}
