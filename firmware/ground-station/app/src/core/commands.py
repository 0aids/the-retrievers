...
def handle_command(cmd: int, args: int | None = None):
    ...
    data += next(sequence_number).to_bytes(length=2, byteorder="little")
...
# it does a block in the write loop that checks if most recent ack is bigger than most recent sequence number in the queue
#   if it is: item is finally popped from the queue
#   if not: item is popped from the queue, ground station sends same request with same number, and then ads back to the queue
